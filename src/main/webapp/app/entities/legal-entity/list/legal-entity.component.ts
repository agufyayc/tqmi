...
import { EntityArrayResponseType, LegalEntityService } from '../service/legal-entity.service';
...
export class LegalEntityComponent implements OnInit {
  ...
  trackId = (_index: number, item: ILegalEntity): number => this.legalEntityService.getLegalEntityIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.legalEntities = this.refineData(dataFromBody);
  }

  protected refineData(data: ILegalEntity[]): ILegalEntity[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: ILegalEntity[] | null): ILegalEntity[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.legalEntityService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
